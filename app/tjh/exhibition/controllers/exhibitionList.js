...
angular.module('app.hotel')
    .controller('exhibitionListCtr', function ($scope,$http,$state,AlertService,$log,Upload,oss,$window) {
        //    获取展馆列表
        ...
        hotelPromise.then(function (res) {
            ...
            if(res.data.code!=0){
                ...
                $scope.exhibitionlData=res.data.data.content;
                AlertService.success('展馆列表加载成功！');
            }
        })
        ...
        $scope.pageChanged = function() {
            ...
            $http.get('tjh/exhibitionHotel',{params:{page:$scope.currentPage,type:2}})
                .then(function(res){
                    if(res.data.code==0){
                        $scope.exhibitionlData=res.data.data.content;
                    }
                ...
                $http.delete("tjh/exhibitionHotel/"+item.id+"")
                    .then(function (res) {
                        if(res.data.code==0){
                            ...
                            $scope.exhibitionlData.splice(idx,1);
                        }
                    })
            })
        }
